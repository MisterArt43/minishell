...
char	*ft_strdup(const char *s1, t_gc **gc)
{
	...
	i = 0;
	if (gc != NULL)
		cpy = (char *)ft_gc_add_back(gc, ft_gc_new(malloc((ft_strlen(s1) + 1) * \
	sizeof(char)), "An error occur while mallocing"));
	else
...
char	*ft_substr(char const *s, unsigned int start, size_t len, t_gc **gc)
{
	...
	while (s[start + i] && i != len)
		i++;
	if (gc != NULL)
		res = (char *)ft_gc_add_back(gc, ft_gc_new(malloc((i + 1) * sizeof(char)), \
	"An error occur while mallocing"));
	else
